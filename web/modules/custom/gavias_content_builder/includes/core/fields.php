...
class fields {
   ...
   * Render field text.
   */
  public function renderFieldText($field = [], $value = '') {
    $output = '';
    if (is_array($value)) {
      $value = '';
    }
  ...
  /**
   * Render field textlangs.
   */
  public function renderFieldTextLangs($field = [], $value = '') {
    $_id = gaviasBuilderMakeId();
    ...
    print '<li><a class="active" href="#' . $_id . 'gvalangdefault">' . $this->t('Default') . '</a></li>';
   ...
   * Render field select.
   */
  public function renderFieldSelect($field = [], $value = '') {
    $output = '';
   ...
   */
  public function renderFieldTextArea($field = [], $value = '') {
    $class = $field['class'] ?? '';
    if (isset($field['shortcodes']) && $field['shortcodes']) {
      $class .= 'gerenal_sc';
    }
    $param = $field['param'] ?? '';
    ob_start();
  ...
  /**
   * Render field textarealangs.
   */
  public function renderFieldTextAreaLangs($field = [], $value = '') {
    $_id = gaviasBuilderMakeId();
    $languages = \Drupal::languageManager()->getLanguages();
    if ($value && !is_array($value)) {
      $value = [
  ...
  /**
   * Render field textarea no html.
   */
  public function renderFieldTextAreaNoHtml($field = [], $value = '') {
    $class = $field['class'] ?? '';
    if (isset($field['shortcodes']) && $field['shortcodes']) {
      $class .= 'gerenal_sc';
    }
   ...
   * Render field upload.
   */
  public function renderFieldUpload($field = [], $value = '') {
    if (gavias_builder_imce()) {
      return $this->renderFieldUploadImce($field, $value);
    }
    else {
      return $this->renderFieldUploadNonImce($field, $value);
    }
  }

  /**
   * Render field upload imce.
   */
  public function renderFieldUploadImce($field = [], $value = '') {
    $_id = gaviasBuilderMakeId(10);
  ...
  /**
   * Render field upload non imce.
   */
  public function renderFieldUploadNonImce($field = [], $value = '') {
    $_id = gaviasBuilderMakeId(10);
    $default_image = base_path() . GAVIAS_BLOCKBUILDER_PATH . '/assets/images/default.png';
    if ($value) {
      $path_image_demo = substr(base_path(), 0, -1) . $value;
   ...
   * Render field info.
   */
  public function renderFieldInfo($field = [], $value = '') {
    $output = '';
   ...
   * Render field tabs.
   */
  public function renderFieldTabs($field = [], $value = '') {
    // $class = (isset($field['class'])) ? $field['class'] : '';
    $name = $field['id'];
