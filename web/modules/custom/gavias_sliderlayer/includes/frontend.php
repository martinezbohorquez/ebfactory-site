...
/**
 * Gavias sliderlayerblock_content!
 */
function gavias_sliderlayer_block_content($sid) {
  ...
  $ss->sliderLayout = $settings->sliderlayout ?? 'auto';
  ...
  $ss->disableProgressBar = $settings->progressbar_disable ?? "on";
  $ss->stopAtSlide = -1;
  $ss->stopAfterLoops = -1;
  $ss->shadow = $settings->shadow ?? 0;
  $ss->dottedOverlay = $settings->dottedOverlay ?? 'none';
  $ss->startDelay = 0;
  $ss->lazyType = "none";
  $ss->spinner = "spinner0";
  $ss->shuffle = "off";
  ...
  $ss->navigation->onHoverStop = $settings->onhoverstop ?? 'on';
  ...
  $ss->navigation->arrows->left->h_align = $settings->navigationLeftHAlign ?? 'left';
  $ss->navigation->arrows->left->v_align = $settings->navigationLeftVAlign ?? 'center';
  ...
  $ss->navigation->arrows->right->h_align = $settings->navigationRightHAlign ?? 'right';
  $ss->navigation->arrows->right->v_align = $settings->navigationRightVAlign ?? 'center';
...
function gavias_sliderlayer_slide($vars, $index) {
  ...
  if (!isset($slide->scalestart)) {
    $slide->scalestart = 0;
  }
  if (!isset($slide->scaleend)) {
    $slide->scaleend = 0;
  }
  if (!isset($slide->data_parallax)) {
    $slide->data_parallax = 0;
  }
  if (!$slide->background_color) {
    $slide->background_color = '#f2f2f2';
  }
  $data_kenburns = 'off';
  ...
  if (isset($slide->video_source) && (isset($slide->youtube_video) || isset($slide->vimeo_video) || isset($slide->html5_mp4))) {
    if ($slide->video_source &&($slide->youtube_video || $slide->vimeo_video || $slide->html5_mp4)) {
      ...
      if ($slide->video_source == 'html5') {
        $vars['attributes_video_array']['data-nextslideatend'] = $slide->mp4_nextslideatend ?? TRUE;
        $vars['attributes_video_array']['data-videoloop'] = $slide->mp4_videoloop ?? 'loopandnoslidestop';
      }
      ...
      // $vars['attributes_video_array']['data-videoloop'] =
      // 'loopandnoslidestop';
  ...
  switch ($layer->type) {
    ...
    case 'image':
      ...
      if ($layer->link) {
        ...
        /* $vars['content'] = "<div class=\"layer-style-image\" style=\"width:
         *{$width}px; height: auto;\"> <img alt=\"\"
         *src=\"{$path_image}{$image}\"/></div>";
         */
